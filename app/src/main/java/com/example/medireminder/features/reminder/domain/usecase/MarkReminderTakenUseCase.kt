...
class MarkReminderTakenUseCase @Inject constructor(
    ...
    suspend operator fun invoke(reminderId: String, scheduledTime: Long): Result<Unit> {
        return try {
            ...
            if (repository.hasReminderLog(reminderId, scheduledTime, ReminderAction.TAKEN)) {
                return Result.failure(Exception("Reminder already marked taken"))
            }
            ...
            repository.addReminderLog(
                ...
                actionTaken = ReminderAction.TAKEN
            )
